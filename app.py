    ...
    if st.session_state.current_image_obj:
            ...
            if not LANDING_API_KEY:
                ...
                if r.ok:
                    ...
                    for det in preds:
                        ...
                        left = b.get('x', 0) * w
                        top = b.get('y', 0) * h
                        width = b.get('width', 0) * w
                        height = b.get('height', 0) * h
        ...
        if canvas and hasattr(canvas, 'json_data') and canvas.json_data:
            ...
            for obj in objs:
                ...
                txt = next((a['text'] for a in st.session_state.annotations if a['id'] == ann_id), '')
                updated.append({'rect': rct, 'text': txt or 'Annotation', 'id': ann_id})
